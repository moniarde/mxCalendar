<?php
$_lang['mxcalendars'] = 'mxCalendar';
$_lang['mxcalendars.desc'] = 'Beheer hier je agenda\'s.';
$_lang['mxcalendars.name'] = 'Naam';
$_lang['mxcalendars.description'] = 'Omschrijving';
$_lang['mxcalendars.management'] = 'mxCalendar Manager';
$_lang['mxcalendars.management_desc'] = 'Beheer hier je agenda en algemene instellingen. Je kunt dit doen door te dubbelklikken op het raster, of door rechts te klikken op de respectievelijke rij voor aanvullende menu opties.';
$_lang['mxcalendars.event_title_create'] = 'Maak nieuw evenement';
$_lang['mxcalendars.event_title_update'] = 'Actualiseer evenement';
$_lang['mxcalendars.event_title_duplicate'] = 'Dupliceer evenement';
$_lang['mxcalendars.event_title_remove'] = 'Verwijder evenement';
$_lang['mxcalendars.tab_events'] = 'Agendapunt';
$_lang['mxcalendars.tab_categories'] = 'Categorieën';
$_lang['mxcalendars.tab_settings'] = 'Global Settings';
$_lang['mxcalendars.tab_calendars'] = 'Agenda\'s';
$_lang['mxcalendars.grid_col_context'] = 'Site (context)';
$_lang['mxcalendars.grid_col_title'] = 'Titel';
$_lang['mxcalendars.grid_col_category'] = 'Categorie';
$_lang['mxcalendars.grid_col_start'] = 'Start datum/tijd';
$_lang['mxcalendars.grid_col_end'] = 'Eind datum/tijd';
$_lang['mxcalendars.grid_col_repeate'] = 'Herhalend/Terugkerend';
$_lang['mxcalendars.grid_col_formchunk'] = 'Formulier';
$_lang['mxcalendars.txt_search_default'] = 'Zoek ...';
$_lang['mxcalendars.btn_display_past_events'] = 'Terugblik';
$_lang['mxcalendars.btn_create'] = 'Maak nieuw agenda item';
$_lang['mxcalendars.label_product'] = 'Product';
$_lang['mxcalendars.label_industry'] = 'Industrie';
$_lang['mxcalendars.label_select_context'] = 'Selecteer een site (context) of laat leeg voor allen';
$_lang['mxcalendars.label_select_calendar'] = 'Selecteer een agenda';
$_lang['mxcalendars.label_today'] = 'Vandaag';
$_lang['mxcalendars.label_repeating_event'] = 'Herhalend evenement';
$_lang['mxcalendars.label_repeat_type'] = 'Vindt plaats';
$_lang['mxcalendars.label_repeaton']='Vindt plaats op';
$_lang['mxcalendars.label_repeat_frequency'] = 'Herhaald elke';
$_lang['mxcalendars.label_repeat_last_occurance'] = 'Laatse keer';
$_lang['mxcalendars.label_description'] = 'Omschrijving';
$_lang['mxcalendars.label_location'] = 'Locatie';
$_lang['mxcalendars.label_name'] = 'Naam';
$_lang['mxcalendars.label_address'] = 'Adres';
$_lang['mxcalendars.label_display_map'] = 'Toon kaart';
$_lang['mxcalendars.label_link'] = 'Link';
$_lang['mxcalendars.label_link_rel'] = 'Link Rel';
$_lang['mxcalendars.label_link_target'] = 'Link Doel';
$_lang['mxcalendars.label_cancel'] = 'Annuleer';
$_lang['mxcalendars.label_save'] = 'Bewaar';
$_lang['mxcalendars.label_sunday'] = 'Zondag';
$_lang['mxcalendars.label_monday'] = 'Maandag';
$_lang['mxcalendars.label_tuesday'] = 'Dinsdag';
$_lang['mxcalendars.label_wednesday'] = 'Woensdag';
$_lang['mxcalendars.label_thursday'] = 'Donderdag';
$_lang['mxcalendars.label_friday'] = 'Vrijdag';
$_lang['mxcalendars.label_saturday'] = 'Zaterdag';
$_lang['mxcalendars.label_dow_7'] = 'zondag';
$_lang['mxcalendars.label_dow_1'] = 'maandag';
$_lang['mxcalendars.label_dow_2'] = 'dinsdag';
$_lang['mxcalendars.label_dow_3'] = 'woensdag';
$_lang['mxcalendars.label_dow_4'] = 'donderdag';
$_lang['mxcalendars.label_dow_5'] = 'vrijdag';
$_lang['mxcalendars.label_dow_6'] = 'zaterdag';
$_lang['mxcalendars.label_daily'] = 'Dagelijks';
$_lang['mxcalendars.label_weekly'] = 'Weeklijks';
$_lang['mxcalendars.label_monthly'] = 'Maandelijks';
$_lang['mxcalendars.label_yearly'] = 'Jaarlijks';
$_lang['mxcalendars.label_forms'] = 'Formulier';
$_lang['mxcalendars.label_images'] = 'Images';
$_lang['mxcalendars.image_filepath'] = 'File Path';
$_lang['mxcalendars.err_ae'] = 'Een agenda item met deze naam bestaat al.';
$_lang['mxcalendars.err_nf'] = 'Agendapunt id niet gevonden.';
$_lang['mxcalendars.err_ns'] = 'Record niet gevonden voor update.';
$_lang['mxcalendars.err_ns_name'] = 'Geef aub een naam voor het agendapunt.';
$_lang['mxcalendars.err_ns_title'] = 'Geef aub een titel voor het agendpunt.';
$_lang['mxcalendars.err_remove'] = 'Bij verwijdering van agendapunt heeft zich een fout voorgedaan.';
$_lang['mxcalendars.err_save'] = 'Bij het opslaan van het agendapunt heeft zich een fout voorgedaan.';
$_lang['mxcalendars.err_event_req_name'] = 'Naam is noodzakelijk';
$_lang['mxcalendars.err_event_req_category'] = 'Categorie is noodzakelijk';
$_lang['mxcalendars.err_event_req_startdate'] = 'Begindatum is noodzakelijk';
$_lang['mxcalendars.err_event_req_starttime'] = 'Begintijd is noodzakelijk';
$_lang['mxcalendars.err_event_req_enddate'] = 'Einddatum is noodzakelijk';
$_lang['mxcalendars.err_event_req_endtime'] = 'Eindtijd is noodzakelijk';
$_lang['mxcalendars.label_select_context_err'] = 'Alle sites';
$_lang['mxcalendars.err_event_req_repeattype'] = 'Herhalingstype is noodzakelijk voor een zich herhalend agendapunt';
$_lang['mxcalendars.err_event_req_repeatfrequency'] = 'Herhaal elke, frequentie, is noodzakelijk voor een zich herhalend agendapunt';
$_lang['mxcalendars.err_event_req_repeatenddate'] = 'Een einddatum is noodzakelijk voor een zich herhalend agendapunt.';
$_lang['mxcalendars.err_event_req_repeaton'] = 'Vindt plaats op (selectie) is noodzakelijk voor wekelijks herhalende agendapunten.';
$_lang['mxcalendars.err_event_req_validcat'] = 'Er is geen geldige id gevonden voor de standaard of gepubliceerde categorie, zorg er aub voor dat een geldige categorie is aangemaakt.';


$_lang['mxcalendars.mxcalendars_remove_confirm'] = 'Weet je zeker dat je dit agendapunt wilt verwijderen?';
$_lang['mxcalendars.downloads'] = 'Downloads';
$_lang['mxcalendars.location'] = 'Locatie';
$_lang['mxcalendars.search_default_text'] = 'Zoek evenementen...';
$_lang['mxcalendars.top_downloaded'] = 'Meest gedownloade agendapunten';
$_lang['mxcalendars.categoryid_col_label'] = 'Categorie';
$_lang['mxcalendars.startdate_col_label'] = 'Start datum';
$_lang['mxcalendars.starttime_col_label'] = 'Start tijd';
$_lang['mxcalendars.enddate_col_label'] = 'Eind datum';
$_lang['mxcalendars.endtime_col_label'] = 'Eind tiid';
$_lang['mxcalendars.repeating_col_label'] = 'Herhalend';
$_lang['mxcalendars.repeating_last_occ_col_label'] = 'Laatste herhaling';

$_lang['mxcalendars.label_startdate'] = 'Start datum/tijd';
$_lang['mxcalendars.label_starttime'] = 'Start tijd';
$_lang['mxcalendars.label_title'] = 'Naam / Titel';
$_lang['mxcalendars.label_enddate'] = 'Eind datum/tijd';
$_lang['mxcalendars.label_duplicate'] = 'Dupliceer';
$_lang['mxcalendars.label_select_category'] = 'Selecteer een categorie';
$_lang['mxcalendars.label_select_category_err'] = 'Selecteer een geldige categorie';
$_lang['mxcalendars.label_calendar_id'] = 'Agenda ID';

$_lang['mxcalendars.label_select_form'] = 'Selecteer een formulier';
$_lang['mxcalendars.label_select_form_err'] = 'Selecteer een geldig formulier chunk';

$_lang['mxcalendars.btn_show_past_events'] = 'Terugblik';
$_lang['mxcalendars.btn_show_upcoming_events'] = 'Vooruitblik';
$_lang['mxcalendars.tip_repeaton'] = 'Herhaal elke ___ (dagen|weken|maanden|jaren).<br /><br />Stel de herhalingsfrequentie in voor het type (dagelijks, weeklijks, maandelijks, jaarlijks) zoals hierboven gekozen.';
//$_lang['mxcalendars.'] = '';

//-- Category Management
$_lang['mxcalendars.category_desc'] = 'Beheer de categorieën waar elk agendapunt aan gekoppeld kan worden. Voor elke categorie is een voor- en achtergrond kleur in te stellen';
$_lang['mxcalendars.label_window_create'] = 'Maak een nieuwe categorie';
$_lang['mxcalendars.label_window_create_cal'] = 'Maak een nieuwe Agenda';
$_lang['mxcalendars.label_window_create_cat'] = 'Maak een nieuwe Categorie';
$_lang['mxcalendars.category_name_col_label'] = 'Naam';
$_lang['mxcalendars.category_isdefault_col_label'] = 'Is standaard';
$_lang['mxcalendars.category_foregroundcss_col_label'] = 'Tekst kleur';
$_lang['mxcalendars.category_backgroundcss_col_label'] = 'Achtergrond kleur';
$_lang['mxcalendars.category_inlinecss_col_label'] = 'Inline CSS';
$_lang['mxcalendars.category_disabled_col_label'] = 'Inactief';
$_lang['mxcalendars.category_active_col_label'] = 'Actief';
$_lang['mxcalendars.cateogry_remove_title'] = 'Verwijder categorie';
$_lang['mxcalendars.cateogry_remove_confirm'] = 'Weet je zeker dat je deze categorie wilt verwijderen?';
$_lang['mxcalendars.btn_create_cat'] = 'Maak een nieuwe categorie';
$_lang['mxcalendars.default_category_search'] = "Zoek categorie...";
$_lang['mxcalendars.menu_update_category'] = 'Update categorie';
$_lang['mxcalendars.menu_remove_category'] = 'Verwijder categorie';

$_lang['mxcalendars.label_category_heading'] = 'Categorieën:';
$_lang['mxcalendars.label_category_viewAll'] = 'Laat allen zien';

//-- Settings friendly names
$_lang['mxcalendars.set_category_required'] = 'Categorie noodzakelijk voor agendapunt';
$_lang['mxcalendars.set_event_desc_type'] = 'Omschrijving veld type';
$_lang['mxcalendars.set_mgr_dateformat'] = 'Datum formaat';
$_lang['mxcalendars.set_mgr_timeformat'] = 'Tijd formaat';

//-- Calendar Management
$_lang['mxcalendars.calendar_desc'] = 'Beheer agenda\'s waar evenementen aan toegevoegd kunnen worden, maak er zoveel als nodig.';

$_lang['mxcalendars.err_ac'] = 'Fout bij opslaan agenda, probeer opnieuw.';
$_lang['mxcalendars.calendar_name_col_label'] = 'Naam';
$_lang['mxcalendars.calendar_name_col_active'] = 'Actief';
$_lang['mxcalendars.calendar_default_calendar_search'] = 'Zoek ...';
$_lang['mxcalendars.calendar_btn_create'] = 'Maak nieuwe agenda';


$_lang['mxcalendars.calendar_context_menu_update'] = 'Update';
$_lang['mxcalendars.calendar_context_menu_remove'] = 'Verwijder';

$_lang['mxcalendars.label_select_calendar'] = 'Selecteer agenda';
$_lang['mxcalendars.label_select_calendar_err'] = 'Agenda niet gevonden';
$_lang['mxcalendars.grid_col_calendar'] = 'Agenda';



?>
